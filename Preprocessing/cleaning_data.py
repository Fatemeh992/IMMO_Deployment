...
class Preprocessing:
    ...
    missing values, encoding, and outlier processing.
    """

    def __init__(self, data_path, output_dir):
        """
        Args:
            data_path (str): Path to the CSV file containing the dataset after scraping.
    ...
    def run_pipeline(self):
        ...
        self.df = self.handle_outliers(self.df, 
                                       columns=['Price', 'Living Area'], method='IQR', remove=True)
    ...
    def handle_missing_values(df):
        ...
        columns_to_drop = ['id', 'Type of property', 'Type of sale', 'Garden', 'Garden area', 
                        ...
                        'Terrace', 'Number of rooms', 'Compound Listing']
    ...
    def visualize_distributions_and_correlation(self):
        """
        Visualizes distributions of key numerical variables
    ...
    def save_to_csv(self):
        ...
        """
        output_file = os.path.join(self.output_dir, f"processed_data.csv")
        self.df.to_csv(output_file, index=False)
        print(f"Processed data saved to {output_file}")


def preprocess(filepath: str, output_dir: str = None):
    """
    Preprocesses the data using the Preprocessing class.
    
    Args:
        filepath (str): Path to the input CSV file.
        output_dir (str, optional): Directory to save the processed file and outputs. Defaults to the current working directory.
    ...
    # Set default for output_dir if not provided
    if not output_dir:
        output_dir = os.getcwd()
    elif not os.path.isdir(output_dir):
        raise ValueError(f"The output directory '{output_dir}' is not a valid directory.")
    elif not os.access(output_dir, os.W_OK):
        raise PermissionError(f"No write permission for the output directory '{output_dir}'.")

    # Initialize preprocessing and run the pipeline
    preprocessing = Preprocessing(data_path=filepath, output_dir=output_dir)
    preprocessing.run_pipeline()


preprocess('Preprocessing/Immoweb_scraping_result.csv', 'Preprocessing')
